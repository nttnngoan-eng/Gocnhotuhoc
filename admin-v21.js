...
function loadSelected(){
  ...
  $('openReaderLink').href=l?`reader.html?id=${encodeURIComponent(l.id)}&v=24.2.8`:'reader.html?v=24.2.8';
...
function upsertFromForm(){
  ...
  $('openReaderLink').href=`reader.html?id=${encodeURIComponent(l.id)}&v=24.2.8`;
  return l;
}
    ...
    const chapters=(b.chapters||[]).map(c=>{
      const lessons=(c.lessons||[])
        ...
        .map(l=>({
          ...
          href:`reader.html?id=${l.id}&v=24.2.8`
        }));
...
$('publishAppearance')?.addEventListener('click',async()=>{
  ...
  try{
    ...
    await apiRequest('/publish-v21',{method:'POST',body:JSON.stringify({dataJs:buildDataJs(),catalogJs:buildCatalogJs(),message:'Cập nhật V24.2.8: icon + tên sách, bỏ bìa sách, thêm icon lá bồ đề non đỏ'})});
...
(function initAppIconMaker(){
  ...
  if(!previewEl) return;
  let source={type:'current', url:'icon-512.png?v=24.2.8'};
  ...
  $('usePresetAppIcon1')?.addEventListener('click',async()=>{
    try{ source={type:'preset',url:'app-icon-hinh-so-1.png?v=24.2.8'}; await blobPreview(); state('Đã chọn Hình số 1 – Lá bồ đề non. Bấm “Tải cả 2 icon” để tạo file.','ok'); }catch(e){state('Không tải được Hình số 1.','err');}
  });
  ...
  $('resetAppIconPreview')?.addEventListener('click',()=>{ source={type:'current',url:'icon-512.png?v=24.2.8'}; if(fileEl)fileEl.value=''; preview(source.url); state('Đang xem icon App hiện tại.'); });
...
(function initBuiltInImageLibrary(){
  ...
  const siteCovers=[{name:'Bồ đề non · Nâu nhạt',url:'cover-bo-de-non-v24.png?v=24.2.8'},...coverDefs.map((_,i)=>makeCover(i,false))];
  ...
  const app=[];
  app.push({name:'Hình số 1',url:'app-icon-hinh-so-1.png?v=24.2.8',type:'preset'});
  const lib=window.GNTT_BOOK_ICONS;
  if(lib){
