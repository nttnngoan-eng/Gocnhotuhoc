...
function loadSelected(){
  ...
  $('openReaderLink').href=l?`reader.html?id=${encodeURIComponent(l.id)}&v=24.2.15r5r5`:'reader.html?v=24.2.15r5r5';
...
function upsertFromForm(){
  ...
  $('openReaderLink').href=`reader.html?id=${encodeURIComponent(l.id)}&v=24.2.15r5r5`;
  return l;
}
    ...
    const chapters=(b.chapters||[]).map(c=>{
      const lessons=(c.lessons||[])
        ...
        .map(l=>({
          ...
          href:`reader.html?id=${l.id}&v=24.2.15r5r5`
        }));
...
$('publishAppearance')?.addEventListener('click',async()=>{
  ...
  try{
    ...
    await apiRequest('/publish-v21',{method:'POST',body:JSON.stringify({dataJs:buildDataJs(),catalogJs:buildCatalogJs(),message:'Cập nhật V24.2.15r5r5: icon + tên sách, bỏ bìa sách, thêm icon lá bồ đề non đỏ'})});
...
(function initAppIconMaker(){
  ...
  if(!previewEl) return;
  let source={type:'current', url:'icon-512.png?v=24.2.15r5r5'};
  ...
  fileEl?.addEventListener('change',async()=>{
    ...
    try{ const u=await fileToUrl(f); source={type:'file',url:u}; await blobPreview(); state('Đã chọn ảnh. Có thể tải icon 180/192/512.','ok'); }catch(e){state('Không đọc được ảnh.','err');}
  });
  $('usePresetAppIcon1')?.addEventListener('click',async()=>{
    try{ source={type:'preset',url:'app-icon-hinh-so-1.png?v=24.2.15r5r5'}; await blobPreview(); state('Đã chọn Hình số 1 – Lá bồ đề non. Bấm “Tải cả 3 icon” để tạo file.','ok'); }catch(e){state('Không tải được Hình số 1.','err');}
  });
  ...
  $('resetAppIconPreview')?.addEventListener('click',()=>{ source={type:'current',url:'icon-512.png?v=24.2.15r5r5'}; if(fileEl)fileEl.value=''; preview(source.url); state('Đang xem icon App hiện tại.'); });
  $('downloadAppIcon180')?.addEventListener('click',()=>download(180));
  $('downloadAppIcon192')?.addEventListener('click',()=>download(192));
  $('downloadAppIcon512')?.addEventListener('click',()=>download(512));
  $('downloadBothAppIcons')?.addEventListener('click',async()=>{ await download(180); setTimeout(()=>download(192),450); setTimeout(()=>download(512),900); });
})();
...
(function initBuiltInImageLibrary(){
  ...
  const siteCovers=[{name:'Bồ đề non · Nâu nhạt',url:'cover-bo-de-non-v24.png?v=24.2.15r5r5'},...coverDefs.map((_,i)=>makeCover(i,false))];
  ...
  const app=[];
  app.push({name:'Hình số 1',url:'app-icon-hinh-so-1.png?v=24.2.15r5r5',type:'preset'});
  const lib=window.GNTT_BOOK_ICONS;
  if(lib){
  ...
  // V24.2.15r5r5 rev2: Mot thu vien chung cho tat ca icon + anh cover.
  ...
  function currentUnifiedStyle(){
    if(unifiedMode==='book') return bookIconStyle?.value||'brown';
  ...
  function renderUnifiedFilters(){
    ...
    if(!groups.includes(unifiedGroup)) unifiedGroup='Tất cả';
    unifiedFilters.innerHTML=groups.map(g=>`<button type="button" data-lib-group="${esc(g)}" class="${g===unifiedGroup?'selected':''}">${esc(g)}</button>`).join('');
  }
  function renderUnifiedGrid(){
    if(!unifiedGrid)return;
    const q=String(unifiedSearch?.value||'').trim().toLowerCase();
    const style=currentUnifiedStyle();
    const items=unifiedItems().filter(x=>(unifiedGroup==='Tất cả'||x.group===unifiedGroup)&&(!q||(`${x.name} ${x.group}`).toLowerCase().includes(q)));
    unifiedGrid.innerHTML=items.map(x=>{
      if(x.kind==='cover') return `<button type="button" class="unified-library-item" data-unified-id="${esc(x.id)}"><img src="${x.url}" alt="${esc(x.name)}"><small>${esc(x.name)}</small></button>`;
      let art='';
      if(x.id==='image-bodhi-red') art='<img src="app-icon-hinh-so-1.png?v=24.2.15r5r5" alt="'+esc(x.name)+'" style="width:64px;height:64px;object-fit:contain">';
  ...
  function openUnified(mode){
    ...
    renderUnifiedFilters(); renderUnifiedGrid(); if(unifiedModal)unifiedModal.hidden=false;
  }
  $('openUnifiedLibraryCover')?.addEventListener('click',()=>openUnified('cover'));
  $('openUnifiedLibraryApp')?.addEventListener('click',()=>openUnified('app'));
  $('openUnifiedLibrarySite')?.addEventListener('click',()=>openUnified('site'));
  $('openUnifiedLibraryBook')?.addEventListener('click',()=>openUnified('book'));
  $('closeUnifiedLibrary')?.addEventListener('click',()=>{if(unifiedModal)unifiedModal.hidden=true;});
  unifiedModal?.addEventListener('click',e=>{if(e.target===unifiedModal)unifiedModal.hidden=true;});
  ...
  unifiedGrid?.addEventListener('click',e=>{
    ...
    if(unifiedMode==='cover'){
      ...
      const detail=id==='image-bodhi-red'?{name:item?.label||'Lá bồ đề non đỏ',url:'app-icon-hinh-so-1.png?v=24.2.15r5r5',type:'preset'}:{name:item?.label||id,url:svgUrl(window.GNTT_BOOK_ICONS.svg(id)),type:'site'};
      window.dispatchEvent(new CustomEvent('gntt-app-preset',{detail}));
    }
    if(unifiedModal)unifiedModal.hidden=true;
  });
})();
