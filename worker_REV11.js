// Góc nhỏ tu học - V21.1.1 Publisher (bài đọc + PDF)
...
async function deleteFile(path,message,env){
  ...
  const current=await gh(`/contents/${encoded}?ref=${encodeURIComponent(branch)}`,env,{method:"GET"});
  return gh(`/contents/${encoded}`,env,{method:"DELETE",headers:{"Content-Type":"application/json"},body:JSON.stringify({message,sha:current.sha,branch})});
}
...
export default{
  async fetch(request,env){
    ...
    if(url.pathname==="/health"&&request.method==="GET"){
      return json({ok:true,service:"gocnhotuhoc-publisher-rev11",repo:`${env.GITHUB_OWNER}/${env.GITHUB_REPO}`},200,env);
    }


    if(url.pathname==="/publish-pdf-v21-1-1"&&request.method==="POST"){
    ...
    // REV11: xóa vĩnh viễn file PNG icon khỏi GitHub. Chỉ cho phép file icon ở thư mục gốc.
    if(url.pathname==="/delete-icon-v11"&&request.method==="POST"){
      let p;try{p=await request.json()}catch{return json({error:"Dữ liệu xóa icon không hợp lệ"},400,env)}
      const file=String(p.file||"");
      if(!/^(gold|pink)-\d{2}\.png$/i.test(file))return json({error:"REV11 chỉ xóa vĩnh viễn các PNG icon gold/pink an toàn"},400,env);
      try{
        const r=await deleteFile(file,String(p.message||`Xóa icon ${file}`).slice(0,180),env);
        return json({ok:true,file,commit:r?.commit?.sha||null},200,env);
      }catch(e){return json({error:"GitHub: "+(e?.message||"Không xóa được icon")},502,env)}
    }
