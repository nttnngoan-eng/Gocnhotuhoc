...
(()=>{
 ...
 function renderBooks(filter=''){
   ...
   grid.innerHTML=books.map(b=>{
     ...
     return `<a class="v24-book-card v242-icon-book" href="library.html?v=24.2.3&book=${encodeURIComponent(b.id)}"><div class="v242-book-icon">${icon}</div><div class="v242-book-info"><div class="v24-book-name">${esc(b.title)}</div>${credit}<div class="v24-book-lessons">${count} bài học</div></div></a>`
   }).join('');
 ...
 document.addEventListener('DOMContentLoaded',()=>{
   const cover=document.getElementById('homeCover');if(cover){const st=(CAT.siteSettings||{});cover.querySelectorAll('.v242-cover-blur,.v242-cover-full').forEach(x=>x.remove());if(st.coverImage){cover.style.backgroundImage='none';cover.classList.add('has-cover');cover.classList.remove('default-cover');const blur=document.createElement('div');blur.className='v242-cover-blur';blur.style.backgroundImage=`url(${JSON.stringify(st.coverImage)})`;const img=document.createElement('img');img.className='v242-cover-full';img.src=st.coverImage;img.alt='Ảnh bìa trang chủ';cover.append(blur,img);}else{cover.style.backgroundImage=`url('cover-bo-de-non-v24.png?v=24.2.3')`;cover.classList.remove('has-cover');cover.classList.add('default-cover');}}
