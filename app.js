...
(()=>{
 ...
 function coverTitleHtml(title){const raw=String(title||'').trim();const n=norm(raw);if(n.includes('phap un tuc luan')){let first=raw.replace(/pháp\s+uẩn\s+túc\s+luận/i,'').replace(/[\s–—:-]+$/,'').trim()||'A-TỲ-ĐẠT-MA';return `<span>${esc(first)}</span><span>PHÁP UẨN TÚC LUẬN</span>`;}if(n.includes('tu niem xu')&&n.includes('giang nghia'))return `<span>KINH TỨ NIỆM XỨ</span><span>GIẢNG NGHĨA</span>`;return `<span>${esc(raw)}</span>`;}
 function renderBooks(filter=''){
   const grid=document.getElementById('bookGrid');if(!grid)return;
   const q=norm(filter);
   const fallback=['book-covers-v241/sen-ho.svg','book-covers-v241/sach-sen.svg','book-covers-v241/thien-nau.svg','book-covers-v241/bo-de-hong.svg','book-covers-v241/bo-de-nau.svg','book-covers-v241/bo-de-xanh.svg'];
   const books=(CAT.books||[]).filter(b=>!q||norm(`${b.title} ${b.description||''} ${b.creditName||b.author||''}`).includes(q)).slice(0,10);
   grid.innerHTML=books.map((b,i)=>{const count=(b.chapters||[]).reduce((n,ch)=>n+(ch.lessons||[]).length,0);const cover=b.coverImage||fallback[i%fallback.length];const role=b.creditRole||((b.author||'')?'author':'none');const name=b.creditName||b.author||'';const credit=(role!=='none'&&name)?`<div class="v24-book-credit">${esc(creditLabel(role))}: ${esc(name)}</div>`:'';return `<a class="v24-book-card" href="library.html?v=24.1&book=${encodeURIComponent(b.id)}"><div class="cover"><img src="${esc(cover)}" alt="Bìa ${esc(b.title)}"><div class="v24-cover-title">${coverTitleHtml(b.title)}</div></div><div class="v24-book-name">${esc(b.title)}</div>${credit}<div class="v24-book-lessons">${count} bài học</div></a>`}).join('');
 ...
 document.addEventListener('DOMContentLoaded',()=>{
   const cover=document.getElementById('homeCover');if(cover){const st=(CAT.siteSettings||{});cover.style.backgroundImage=st.coverImage?`url(${JSON.stringify(st.coverImage)})`:`url('cover-bo-de-non-v24.png?v=24.1')`;}
